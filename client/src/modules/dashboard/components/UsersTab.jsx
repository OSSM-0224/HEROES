                ...
                <TableRow key={uId} className="hover:bg-slate-50 border-b border-slate-100">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-bold text-slate-900 text-sm flex items-center gap-1.5">
                          {u.name}
                          {isSelf && (<span className="text-[10px] bg-slate-100 text-slate-600 font-extrabold px-1.5 py-0.5 rounded border border-slate-200">You</span>)}
                        </div>
                        <div className="text-[11px] text-slate-400 font-medium">Joined: {new Date(u.createdAt || Date.now()).toLocaleDateString()}</div>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell className="text-xs font-medium text-slate-800">{u.email}</TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1.5">
                      {u.status === 'ACTIVE' ? (
                        <><CheckCircle2 className="w-4 h-4 text-emerald-600" /><span className="text-xs font-bold text-emerald-700">Active</span></>
                      ) : (
                        <><XCircle className="w-4 h-4 text-rose-600" /><span className="text-xs font-bold text-rose-700">Inactive</span></>
                      )}
                    </div>
                  </TableCell>
