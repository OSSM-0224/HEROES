...
import { DollarSign, Users, CheckCircle, Target, Sparkles, TrendingUp, ArrowUpRight } from 'lucide-react';
import { useOverviewAnimations } from '../hooks/useOverviewAnimations.js';

export const OverviewTab = ({ metrics, onNavigateToLeads }) => {
  const { containerRef } = useOverviewAnimations(metrics);
  ...
  const cards = [
    { title: 'Total Pipeline Value', rawVal: `$${pipelineVal}`, subtitle: 'Combined value of active deals', icon: DollarSign, color: 'text-emerald-700 bg-emerald-50 border-emerald-200' },
    { title: 'Total Leads Captured', rawVal: `${totalLeads}`, subtitle: 'All historical leads in CRM', icon: Users, color: 'text-teal-700 bg-teal-50 border-teal-200' },
    { title: 'Qualified Prospects', rawVal: `${qualifiedCount}`, subtitle: 'High intent leads in stage', icon: Target, color: 'text-emerald-700 bg-emerald-50 border-emerald-200' },
    { title: 'Closed Deals Won', rawVal: `${wonCount}`, subtitle: 'Successfully won contracts', icon: CheckCircle, color: 'text-teal-700 bg-teal-50 border-teal-200' },
  ];
  ...
  return (
    <div ref={containerRef} className="space-y-8 animate-in fade-in duration-300">
      <div className="p-6 bg-gradient-to-r from-emerald-700 via-emerald-600 to-teal-700 rounded-2xl text-white shadow-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border border-emerald-800">
        ...
        <Button onClick={onNavigateToLeads} size="sm" className="bg-white text-emerald-800 hover:bg-slate-100 font-extrabold gap-1.5 shadow-sm border border-emerald-100">
          View Leads Pipeline
          ...
          return (
            <Card key={i} className="border-slate-200 bg-white shadow-2xs hover:shadow-md transition-all rounded-2xl">
              <CardContent className="p-5 space-y-3">
                ...
                <div data-value={card.rawVal} className="stat-count text-3xl font-black text-slate-900 font-mono">{card.rawVal}</div>
      ...
      <Card className="border-slate-200 bg-white shadow-2xs rounded-2xl">
        <CardHeader className="pb-3 border-b border-slate-100">
          <CardTitle className="text-base font-bold text-slate-900 flex items-center justify-between font-heading">
            ...
            <Badge variant="outline" className="text-xs font-mono font-bold bg-slate-50 text-slate-700 border-slate-200">Total Leads: {totalLeads}</Badge>
          </CardTitle>
        </CardHeader>
        <CardContent className="pt-6">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
            {[
              ...
              return (
                <div key={status} className="p-3.5 bg-slate-50 rounded-2xl border border-slate-200 space-y-2 hover:bg-white transition-all shadow-2xs">
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] font-bold text-slate-700 truncate">{status}</span>
