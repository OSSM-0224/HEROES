    ...
    <div className="space-y-6 animate-in fade-in duration-300">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex items-center bg-slate-100 p-1 rounded-xl border border-slate-200">
              <button onClick={() => setViewMode('table')}
                className={`p-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${viewMode === 'table' ? 'bg-white text-emerald-700 shadow-xs' : 'text-slate-500 hover:text-slate-900'}`}
                title="Table View">
                <TableIcon className="w-4 h-4" />
              </button>
              <button onClick={() => setViewMode('kanban')}
                className={`p-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${viewMode === 'kanban' ? 'bg-white text-emerald-700 shadow-xs' : 'text-slate-500 hover:text-slate-900'}`}
          ...
          <Table>
            ...
            <TableBody>
              {leads.length > 0 ? leads.map((lead) => (
                <TableRow key={lead._id || lead.id} onClick={() => onSelectLead(lead)}
                  ...
                  <TableCell><Badge className={STATUS_COLORS[lead.status] || 'bg-slate-100 text-slate-700'}>{lead.status}</Badge></TableCell>
                  <TableCell><SlaCountdownChip createdAt={lead.createdAt} /></TableCell>
                  <TableCell><Badge className={PRIORITY_COLORS[lead.priority] || 'bg-slate-100 text-slate-700'}>{lead.priority}</Badge></TableCell>
                  <TableCell className="font-bold text-slate-900 font-mono text-sm">${lead.value?.toLocaleString() || 0}</TableCell>
                  <TableCell>
                    {lead.assignedTo ? (
                  ...
                  <TableCell className="text-right">
                    <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); onSelectLead(lead); }}
            ...
            return (
              <div key={status} className="bg-slate-100 p-3 rounded-2xl border border-slate-200 space-y-3 min-w-[220px]">
                <div className="flex items-center justify-between px-1">
                  ...
                  <Badge variant="outline" className="text-[10px] font-mono font-bold bg-white text-slate-700 border-slate-300">{stageLeads.length}</Badge>
                </div>
                <div className="space-y-2 max-h-[600px] overflow-y-auto pr-1">
                  {stageLeads.length > 0 ? stageLeads.map((lead) => (
                    <Card key={lead._id || lead.id} onClick={() => onSelectLead(lead)}
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <Badge className={`text-[9px] px-1.5 py-0 ${PRIORITY_COLORS[lead.priority] || ''}`}>{lead.priority}</Badge>
                      </div>
                    ...
                    <div className="text-center py-6 text-[11px] text-slate-400 font-medium border border-dashed border-slate-300 rounded-xl">Empty stage</div>
                  )}
                </div>
              </div>
            );
      ...
      {pagination && totalPages > 1 && (
        <div className="flex items-center justify-between p-4 bg-white border border-slate-200 rounded-2xl text-xs text-slate-600 font-medium">
          <div>Showing page <span className="font-bold text-slate-900">{pagination.page}</span> of{' '}
