...
export const LeadDetailModal = ({
  ...
  const handleStatusChange = (val) => {
    onUpdate(lead._id || lead.id, { status: val });
  };

  const handlePriorityChange = (val) => {
    onUpdate(lead._id || lead.id, { priority: val });
  };

  const handleAssigneeChange = (val) => {
    onUpdate(lead._id || lead.id, { assignedTo: val === 'UNASSIGNED' ? null : val });
  };
  ...
  const handleNoteSubmit = async (e) => {
    ...
    await onAddNote(lead._id || lead.id, newNote);
    setNewNote('');
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl bg-white border-slate-200 text-slate-900 max-h-[90vh] overflow-y-auto p-6 space-y-6 rounded-3xl">
        ...
        <DialogHeader className="space-y-2 text-left border-b border-slate-100 pb-4">
          ...
          <DialogTitle className="text-2xl font-black tracking-tight text-slate-900 font-heading">
            {lead.name}
          </DialogTitle>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-2">
          ...
          <div className="space-y-1">
            ...
            <Select
              value={lead.assignedTo?._id || lead.assignedTo?.id || 'UNASSIGNED'}
        ...
        <div className="space-y-2 pt-4 border-t border-slate-100">
          ...
          <form onSubmit={handleNoteSubmit} className="space-y-2">
            ...
            <div className="flex justify-end">
              <Button type="submit" size="sm" loading={submittingNote} disabled={!newNote.trim()} className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs gap-1.5">
        ...
        <div className="space-y-3 pt-4 border-t border-slate-100">
          <h4 className="text-xs font-bold text-slate-900 uppercase tracking-wider flex items-center gap-2 font-heading">
            ...
            Immutable Activity Audit Log ({logs.length})
          </h4>
        ...
        <DialogFooter className="flex items-center justify-between pt-4 border-t border-slate-100">
          ...
          <Button variant="outline" size="sm" onClick={onClose} className="font-bold text-xs border-slate-200">
            Close
          </Button>
        </DialogFooter>
      </DialogContent>
